...
const normalizeHero = (entry) => {
  ...
  // Ensure synergies exists as an array (defensive)
  const synergies = Array.isArray(hero.synergies) ? hero.synergies : [];

  return { ...hero, synergies };
};

export const sortMembersByPosition = (members = []) =>
  members.slice().sort((a, b) => (a?.position || 0) - (b?.position || 0));

export const getRoleCounts = (entries = []) => {
  return entries.reduce((acc, entry) => {
    const hero = normalizeHero(entry);
    if (!hero?.role) return acc;
