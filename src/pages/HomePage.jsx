...
import { sortMembersByPosition, getRoleCounts, findMutualSynergies } from '../utils/teamInsights';
...
function HomePage() {
  ...
  useEffect(() => {
    ...
    async function fetchTopTeams() {
      try {
        ...
        // Top 5 by views
        const sorted = [...teams].sort((a, b) => (b.views || 0) - (a.views || 0)).slice(0, 5);

        if (alive) setTopTeams(sorted);
      } catch (error) {
        console.error('Error fetching top teams:', error);
        if (alive) setTopTeams([]);
      } finally {
  ...
  async function toggleVote(slug) {
    ...
    try {
      ...
      setTopTeams((prev) =>
        prev.map((t) =>
          t.slug === slug
            ? { ...t, upvote_count: res.data?.upvotes ?? t.upvote_count, user_has_voted: res.data?.voted ?? t.user_has_voted }
            : t
        )
      );
    } catch (e) {
  ...
  const hasTeams = useMemo(() => topTeams && topTeams.length > 0, [topTeams]);
    ...
    <div className="min-h-screen">
      ...
      <section className="relative py-32 overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute -top-24 left-1/2 -translate-x-1/2 w-[900px] h-[900px] rounded-full bg-[#D4AF37]/10 blur-3xl" />
          <div className="absolute -bottom-40 left-1/3 w-[700px] h-[700px] rounded-full bg-[#C5A028]/8 blur-3xl" />
        </div>
          ...
          <div className="text-center max-w-5xl mx-auto fade-in-up">
            <h1 className="heading-hero font-display mb-8">
              <span
                ...
                style={{
                  textShadow:
                    '0 0 80px rgba(255, 215, 0, 0.9), 0 0 40px rgba(212, 175, 55, 0.7), 0 6px 30px rgba(0, 0, 0, 0.9)',
                }}
              >
                Marvel Rivals
              </span>
              <span
                className="text-[#F5F3F0] block mt-2"
                style={{ textShadow: '0 6px 30px rgba(0, 0, 0, 0.9), 0 0 60px rgba(212, 175, 55, 0.2)' }}
              >
                Team Builder
              </span>
            </h1>
            ...
            <div className="flex items-center justify-center gap-6 flex-wrap">
              <Link to="/team-builder" className="btn-gold-hero shadow-2xl">
                Start Building
              </Link>
              <Link to="/teams" className="link-premium text-base">
                Browse Teams
              </Link>
            </div>
            ...
            <div className="mt-14 flex items-center justify-center gap-4 flex-wrap">
              <span className="px-4 py-2 rounded-full bg-[#0F0D0A]/60 border border-[#D4AF37]/15 text-sm text-[#B8AFA3]">
                ⚡ Synergy insights
              </span>
              <span className="px-4 py-2 rounded-full bg-[#0F0D0A]/60 border border-[#D4AF37]/15 text-sm text-[#B8AFA3]">
                🛡️ Role balance checks
              </span>
              <span className="px-4 py-2 rounded-full bg-[#0F0D0A]/60 border border-[#D4AF37]/15 text-sm text-[#B8AFA3]">
                🏆 Community rankings
              </span>
        ...
        <div className="container mx-auto px-6 mt-16">
          <div className="grid md:grid-cols-3 gap-10">
            ...
            <div className="glass p-10 rounded-2xl text-center hover:scale-105 transition-transform duration-300 corner-ornaments">
              ...
              <p className="text-[#B8AFA3] text-lg leading-relaxed">
                Access comprehensive hero info with roles, tags, and team chemistry data.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="py-28 section-premium">
        <div className="container mx-auto px-6">
          <div className="text-center mb-20">
            <h2 className="heading-section gold-text text-shadow-gold mx-auto">Top 5 Most Popular Teams</h2>
            <p className="text-xl text-[#B8AFA3] mt-6">Recognized by the community for excellence</p>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-20">
              <p className="text-[#B8AFA3] text-lg mb-6">No teams yet. Be the first to create a legendary composition!</p>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {topTeams.map((team, index) => {
                const membersSorted = sortMembersByPosition(team.members || []);
                const synergies = findMutualSynergies(membersSorted);
                const roleCounts = getRoleCounts(membersSorted);

                // Use banners if available; if not, fall back to avatar.
                const getMemberArt = (hero) =>
                  resolveHeroArtwork(hero, 'banner') || resolveHeroArtwork(hero, 'image');

                return (
                  <Link
                    key={team.id ?? team.slug ?? index}
                    to={`/teams/${team.slug}`}
                    className="card-elegant gold-shine overflow-hidden group block relative"
                  >
                    {/* Rank Badge */}
                    <div
                      className={`absolute top-4 left-4 w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold border-2 z-10 transition-all duration-300 ${
                        index === 0
                    ...
                    <div className="relative p-4">
                      ...
                      <div className="flex justify-center gap-2 relative">
                        {membersSorted.slice(0, 6).map((member, idx) => {
                          const hero = member?.hero;
                          const src = hero ? getMemberArt(hero) : null;

                          return (
                            <div key={`${team.slug}-${idx}`} className="relative">
                              <div className="w-16 h-20 rounded overflow-hidden opacity-40 group-hover:opacity-65 transition-opacity bg-[#1A1612] border border-[#D4AF37]/10">
                                {src ? (
                                  <img
                                    src={src}
                                    alt={hero?.name || 'Hero'}
                        ...
                        {synergies.length > 0 ? (
                          <div className="flex flex-wrap gap-1">
                            {synergies.slice(0, 2).map((syn, i) => (
                              <span
                                key={i}
                                className="text-xs bg-[#D4AF37]/10 text-[#D4AF37] px-2 py-1 rounded border border-[#D4AF37]/30"
                              >
                                {syn}
                              </span>
                            ))}
                            {synergies.length > 2 && <span className="text-xs text-[#8B8278]">+{synergies.length - 2} more</span>}
                          </div>
                        ) : (
          ...
          {hasTeams && (
            <div className="text-center mt-16">
              <Link to="/teams" className="link-premium text-lg">
                View All Teams <span>→</span>
              </Link>
            </div>
          )}
        </div>
      ...
      <section className="py-32 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="glass p-16 rounded-3xl text-center max-w-4xl mx-auto corner-ornaments">
            <h2 className="text-5xl md:text-6xl font-bold gold-text mb-8 text-shadow-gold">Ready to Build Your Team?</h2>
            ...
            <Link to="/team-builder" className="px-14 py-6 rounded-full btn-gold text-lg inline-block">
              Start Building Now
            </Link>

            <div className="mt-8 text-sm text-[#8B8278]">
              Tip: save teams with a clear name + strategy so others can learn from your build.
            </div>
          </div>
        </div>
      </section>
